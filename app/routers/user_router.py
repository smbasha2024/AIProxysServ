...
from app.configs.dependencies import get_user_service
...
@userRoutes.post("/")
def create_user(user: UserDTO, service: UserServ = Depends(get_user_service)):
...
def read_user(user_id: int, service: UserServ = Depends(get_user_service)):
    result = service.readUser(user_id)
    return result

# In FastAPI route --  Get all users:
@userRoutes.get("/", response_model=list[UserDTO])
def read_all_users(service: UserServ = Depends(get_user_service), skip: int = 0, limit: int = 100):
    result = service.readUsers()
    return result

# In FastAPI route --  Update user by ID:
@userRoutes.put("/{user_id}", response_model=UserDTO)
def update_user(user_id: int, user: UserDTO, service: UserServ = Depends(get_user_service)):
    result = service.updateUser(user_id, user)
    return result

# In FastAPI route --  Delete user by ID:
@userRoutes.post("/{user_id}")
def delete_user(user_id: int, service: UserServ = Depends(get_user_service)):
