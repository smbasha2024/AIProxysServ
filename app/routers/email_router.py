...
@emailRoutes.post("/contactEmail")
async def contact_through_email(email: EmailExtraDTO, service: EmailServ = Depends(email_service_dep)):
    result = service.contactThroughEmail(email)
    return result
